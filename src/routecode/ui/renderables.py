...
def get_logo():
    """Return (route_lines, code_lines) for a sleek, modern look."""
    # Refined characters for maximum "Premium" feel
    route = [
        "█▀▀▄ █▀▀█ █  █ ▀█▀ █▀▀█",
        "█▄▄▀ █░░█ █░░█  █  █░▀▀",
        "█  █ ▀▀▀▀ ▀▀▀▀  ▀  ▀▀▀▀",
...
class RouteCodeFace:
    ...
    def __rich__(self):
        frame = ROUTECODE_FACES[self._index % len(ROUTECODE_FACES)]
        self._index += 1
...
def print_welcome_screen(
    ...
    # ── Logo block (dual-tone: dim "route" + accent "code") ────────────────
    route_lines, code_lines = logo
    logo_text = Text()
    logo_text.append("\n")
    for i in range(len(route_lines)):
        logo_text.append(route_lines[i], style="dim")
        logo_text.append("   ")  # gap between words
        logo_text.append(code_lines[i], style="accent")
        logo_text.append("\n")
    ...
    build_line = Text.from_markup(
        f"[accent]{provider.title()}[/accent]  [dim]·[/dim]  [bold white]{model}[/bold white]"
    )
...
def print_status_line(model: str, provider: str, elapsed: Optional[float] = None):
    time_str = f" · {elapsed:.1f}s" if elapsed is not None else ""
    console.print(
        f" [accent]■[/accent] [white]{provider.title()}[/white] · [dim]{model}{time_str}[/dim]"
    )
