...
def _get_platform_asset_name() -> str:
    ...
    if system == "windows":
        return "loom-cli-windows-x86_64.exe"
    elif system == "darwin":
        return (
            "loom-cli-macos-arm64"
            if machine in ("arm64", "aarch64")
            else "loom-cli-macos-x86_64"
        )
    elif system == "linux":
        return "loom-cli-linux-x86_64"
...
def check_for_update() -> UpdateInfo:
    ...
    try:
        ...
        req.add_header("User-Agent", "loomcli-updater")
...
def _pip_update(update_info: UpdateInfo, console=None) -> bool:
    ...
    try:
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "--upgrade", "loomcli"],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        if console:
            console.print(
                "[success]Update installed. Restart Loom to use the new version.[/success]"
            )
        return True
...
def _binary_update(update_info: UpdateInfo, console=None) -> bool:
    tmp_path = None
    try:
        tmp_fd, tmp_path = tempfile.mkstemp(prefix="loom_update_")
        os.close(tmp_fd)
...
def _replace_and_relaunch_windows(new_exe: str, current_exe: str, console=None) -> bool:
    script_fd, script_path = tempfile.mkstemp(suffix=".bat", prefix="loom_install_")
    os.close(script_fd)

    with open(script_path, "w") as f:
        f.write(
            "@echo off\n"
            "echo Loom CLI Updater\n"
            f":retry\n"
    ...
    if console:
        console.print(
            "[success]Update will install on exit. Loom will restart automatically.[/success]"
        )
    return True


def _replace_and_relaunch_unix(new_exe: str, current_exe: str, console=None) -> bool:
    script_fd, script_path = tempfile.mkstemp(suffix=".sh", prefix="loom_install_")
    os.close(script_fd)
