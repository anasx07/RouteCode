...
use super::types::{
    ApiKeyInputStage, ApprovalMode, ModelMenuItem, PROVIDERS, Screen, SettingsMenuItem,
};
                        ...
                        ApiKeyInputStage::ApiKey => {
                            if let Some(provider_id) = app.pending_provider_id.clone() {
                                if provider_id == "vertex" {
                                    ...
                                    app.api_key_input = TextArea::default();
                                    app.api_key_input.set_placeholder_text(" Location (e.g. us-central1)...");
                                } else {
                ...
                if !input_text.trim().is_empty() {
                    if input_text.starts_with('/') {
                        ...
                        app.tasks.spawn(async move {
                            if let Err(e) = orchestrator.run(&mut history, &model, Some(tx), None).await {
    ...
    match mouse.kind {
        MouseEventKind::Moved => {
         ...
         MouseEventKind::ScrollUp => {
            if app.show_menu || app.show_provider_menu || app.show_model_menu || app.show_settings_menu {
                ...
                if app.show_model_menu {
                    while current > 0 && matches!(app.filtered_models.get(current), Some(ModelMenuItem::Header(_))) {
                        current -= 1;
                    }
                } else if app.show_settings_menu {
                    while current > 0 && matches!(app.settings_items.get(current), Some(SettingsMenuItem::Header(_))) {
                        current -= 1;
                    }
                }
        ...
        MouseEventKind::ScrollDown => {
            if app.show_menu || app.show_provider_menu || app.show_model_menu || app.show_settings_menu {
                ...
                if app.show_model_menu {
                    while next < max - 1 && matches!(app.filtered_models.get(next), Some(ModelMenuItem::Header(_))) {
                        next += 1;
                    }
                } else if app.show_settings_menu {
                    while next < max - 1 && matches!(app.settings_items.get(next), Some(SettingsMenuItem::Header(_))) {
                        next += 1;
                    }
                }
...
mod tests {
    ...
    impl AIProvider for MockProvider {
        ...
        async fn ask(&self, _: Arc<Vec<Message>>, _: &str, _: Arc<Option<Vec<serde_json::Value>>>, _: Option<&str>) -> Result<StreamResponse, anyhow::Error> {
